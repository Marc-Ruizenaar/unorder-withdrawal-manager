...
final class WithdrawalRequestsPage {
	...
	public function register_request_handlers(): void {
		...
		add_action( 'admin_post_unordw_approve_withdrawal', array( $this, 'handle_post_approve' ) );
		add_action( 'admin_post_unordw_reject_withdrawal', array( $this, 'handle_post_reject' ) );
	}
	...
	public function enqueue( string $hook_suffix ): void {
		...
		wp_enqueue_style(
			...
			UNORDW_PLUGIN_URL . 'assets/css/admin-withdrawal-requests.css',
			array(),
			UNORDW_VERSION
		);
		wp_enqueue_script(
			self::JS_HANDLE,
			UNORDW_PLUGIN_URL . 'assets/js/admin-withdrawal-requests.js',
			array( 'jquery' ),
			UNORDW_VERSION,
			true
		);
	...
	public function maybe_handle_approve_from_admin_screen(): void {
		...
		if ( ! isset( $_GET['page'], $_GET['unordw_action'] ) || ! isset( $_GET['request_id'] ) ) {
			return;
		}
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Same as above.
		$act = isset( $_GET['unordw_action'] ) ? sanitize_key( (string) wp_unslash( (string) $_GET['unordw_action'] ) ) : '';
	...
	private function run_approve_from_query(): void {
		...
		if ( $request_id < 1 || ! wp_verify_nonce( $nonce, 'unordw_withdrawal_approve_' . $request_id ) ) {
			wp_die( esc_html__( 'Invalid or expired link.', 'un-order' ) );
		}
		if ( ! $this->approve_one( $request_id ) ) {
			$this->redirect_url( $this->get_success_redirect( 'unordw_msg', 'approve_failed' ) );
		}
		$this->redirect_url( $this->get_success_redirect( 'unordw_msg', 'approved' ) );
	}
	...
	public function handle_post_reject(): void {
		...
		Schema::maybe_upgrade();
		check_admin_referer( 'unordw_reject_withdrawal' );

		$request_id = isset( $_POST['request_id'] ) ? absint( (string) wp_unslash( $_POST['request_id'] ) ) : 0;
		$reason     = isset( $_POST['unordw_reject_reason'] ) ? sanitize_textarea_field( (string) wp_unslash( $_POST['unordw_reject_reason'] ) ) : '';
		...
		if ( ! $this->reject_one( $request_id, $reason ) ) {
			$this->redirect_url( $this->get_success_redirect( 'unordw_msg', 'reject_failed' ) );
		}
		$this->redirect_url( $this->get_success_redirect( 'unordw_msg', 'rejected' ) );
	}
	...
	private function process_bulk(): void {
		$method = isset( $_SERVER['REQUEST_METHOD'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) : '';
		if ( 'POST' !== strtoupper( $method ) || ! isset( $_POST['unordw_withdrawal_bulk'] ) ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		check_admin_referer( 'bulk-unordw_withdrawal_requests' );
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		$ids = isset( $_POST['unordw_withdrawal_requests'] ) && is_array( $_POST['unordw_withdrawal_requests'] ) ? array_map( 'absint', $_POST['unordw_withdrawal_requests'] ) : array();
		$ids = array_values( array_filter( $ids ) );
		if ( $ids === array() || ( 'approve' !== $action && 'reject' !== $action ) ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		$reason = isset( $_POST['unordw_bulk_reject_reason'] ) ? sanitize_textarea_field( (string) wp_unslash( (string) $_POST['unordw_bulk_reject_reason'] ) ) : '';
	...
	private function approve_one( int $request_id ): bool {
		...
		add_action(
			...
			static function () use ( $rid ) {
				do_action( 'unordw_withdrawal_approved', $rid );
			},
			1
		);
		return true;
	...
	private function reject_one( int $request_id, string $reason ): bool {
		...
		add_action(
			'shutdown',
			static function () use ( $rid ) {
				do_action( 'unordw_withdrawal_rejected', $rid );
			},
			1
		);
		return true;
	 ...
	 *
	 * @param string $key Query key (e.g. unordw_msg).
	...
	public function render(): void {
		...
		if ( ! empty( $_GET['unordw_msg'] ) ) {
			$this->render_notices( sanitize_key( (string) wp_unslash( (string) $_GET['unordw_msg'] ) ) );
		}
		...
		<div class="wrap">
			...
			<form id="un-order-withdrawal-requests" method="post" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>">
				<?php
				$table->views();
				// Bulk nonce: wp_nonce_field( 'bulk-unordw_withdrawal_requests' ) is output by $table->display() (tablenav top).
				...
				// phpcs:enable WordPress.Security.NonceVerification.Recommended
				?>
				<input type="hidden" name="unordw_withdrawal_bulk" value="1" />
				<input type="hidden" name="unordw_bulk_reject_reason" id="unordw_bulk_reject_reason" value="" />
				<?php
				$table->display();
				?>
			</form>
			<?php
			$this->add_withdrawal_reason_inline_script( $table );
	...
	/**
	 * Passes `window.unordwWithdrawalReasons` (request id => full text) for the long-reason modal via wp_add_inline_script.
	 * Runs during render (after enqueue); still before admin footer prints scripts.
	 */
	private function add_withdrawal_reason_inline_script( WithdrawalRequestsListTable $table ): void {
		...
		wp_add_inline_script(
			self::JS_HANDLE,
			'window.unordwWithdrawalReasons = ' . $json . ';',
			'before'
		);
	}
			...
			<div class="un-order-reject-dialog__panel" role="dialog" aria-modal="true" aria-labelledby="un-order-reject-title">
				...
				<div id="un-order-reject-single">
					<form id="un-order-reject-form" method="post" action="<?php echo $action; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
						<?php wp_nonce_field( 'unordw_reject_withdrawal' ); ?>
						<input type="hidden" name="action" value="unordw_reject_withdrawal" />
						<input type="hidden" name="request_id" id="un-order-reject-request-id" value="" />
						<p>
							<label for="unordw_reject_reason" class="screen-reader-text"><?php esc_html_e( 'Reason', 'un-order' ); ?></label>
							<textarea name="unordw_reject_reason" id="unordw_reject_reason" class="large-text" rows="5" required></textarea>
						</p>
						<p class="submit">
				...
				<div id="un-order-reject-bulk" class="un-order-reject-bulk" hidden>
					<p>
						<label for="unordw_bulk_reject_textarea" class="screen-reader-text"><?php esc_html_e( 'Reason (bulk)', 'un-order' ); ?></label>
						<textarea id="unordw_bulk_reject_textarea" class="large-text" rows="5"></textarea>
					</p>
					<p class="submit">
