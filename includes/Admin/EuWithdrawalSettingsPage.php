 ...
 */
final class EuWithdrawalSettingsPage extends \WC_Settings_Page {

	/**
	 * Constructor: tab id and label.
	 */
	public function __construct() {
		$this->id    = 'un_order_eu_withdrawal';
	...
	public function get_settings( $current_section = '' ): array {
		$settings = array(
			array(
				...
				'id'    => 'un_order_settings_section',
			),
			array(
				'type'              => 'number',
				'id'                => 'un_order_period_days',
		...
		$categories_field = array(
			'type'     => 'multiselect',
			'id'       => 'un_order_excluded_categories',
		...
		$settings = array_merge(
			...
			array(
				...
				array(
					'type' => 'title',
					'id'   => 'un_order_pro_guest_notice',
					'desc' => __(
						'Customers withdraw from <strong>My Account → Orders</strong>. Guest order lookup and the <code>[eu_withdrawal_button]</code> shortcode are added by <strong>Un Order Pro</strong>.',
						'un-order'
					),
				),
				array(
		...
		return (array) apply_filters( 'un_order_get_settings', $settings, $current_section );
	}
